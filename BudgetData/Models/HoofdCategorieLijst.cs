...
namespace Syntra.Data.Models
{
	public class HoofdCategorieLijst
	{
		...
		public bool VulData()
		{
			...
			string data = GetType().GetEmbeddedResource("new.json");
			categ = data;
			if (data.NotEmpty())
			{
				return Import(data);
		...
		public bool addHoofdCategorie(string hc_naam, string sc_naam )
		{
			if ((Members.Where(t => t.Naam == hc_naam).FirstOrDefault())==null)
			{
				...
				HoofdCategorie hc = new HoofdCategorie(index, hc_naam, subcats);
				Members.Add(hc);
				return true;
			}
			else return false;
		...
		public bool addHoofdCategorie(string hc_naam)
		{
			if ((Members.Where(t => t.Naam == hc_naam).FirstOrDefault()) == null)
			{
				...
				var subcats = new List<SubCategorie>();
				var sc_index = index + 1;
				SubCategorie sc = new SubCategorie(sc_index, "Subcategorie 1");
				subcats.Add(sc);
		...
		{
			HoofdCategorie hc = Members.Where(t => t.SubCats.Contains(sc)).FirstOrDefault();
			return hc;
		}
		public HoofdCategorie getHoofdCategorie(string sc_naam)
		{
			//HoofdCategorie hc = Members.Where(t => t.SubCats.Where(t => t.Naam == sc_naam)!=null).FirstOrDefault();
			SubCategorie sc = getSubCategorie(sc_naam);
			return getHoofdCategorie(sc);
		}
